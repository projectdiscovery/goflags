...
func (f *FlagSet) Parse() error {
	...
	f.MergeConfigFile(config) // try to read default config after parsing flags
	return nil
}
...
func (f *FlagSet) generateDefaultConfig() []byte {
	...
	if f.Marshal {
		...
		for k, v := range f.flagKeys {
			flagset[k] = v.defaultValue
		}

		flagsetb, err := yaml.Marshal(flagset)
		if err == nil {
			configBuffer.Write(flagsetb)
	...
	for _, v := range f.flagKeys {
		...
		configBuffer.WriteString("# ")
		configBuffer.WriteString(strings.ToLower(v.usage))
		configBuffer.WriteString("\n")
		configBuffer.WriteString("#")

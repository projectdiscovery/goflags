...
type FlagSet struct {
	...
	// appliedConfigValues distinguishes config-owned state from caller changes.
	appliedConfigValues map[configTargetKey]configValueState
...
func (flagSet *FlagSet) Parse(args ...string) error {
	...
	loadPrimary := fileutil.FileExists(configFilePath)
	if !loadPrimary {
		configData := flagSet.generateDefaultConfig(flagSet.Marshal && len(configFilePaths) == 1)

		configFileDir := flagSet.GetToolConfigDir()
		if !fileutil.FolderExists(configFileDir) {
			if err := fileutil.CreateFolder(configFileDir); err != nil {
				return fmt.Errorf("create config directory %q: %w", configFileDir, err)
			}
		}

		if err := os.WriteFile(configFilePath, configData, permissionutil.ConfigFilePermission); err != nil {
			return fmt.Errorf("create config file %q: %w", configFilePath, err)
		}
	}

	configFilePathsToLoad, err := existingConfigFilePaths(configFilePaths[:len(configFilePaths)-1])
	if err != nil {
		return err
	}
	if loadPrimary {
		configFilePathsToLoad = append(configFilePathsToLoad, configFilePath)
	}
